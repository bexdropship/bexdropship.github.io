...
export default function AppView() {
  ...
  const [latestOrdersShipped, setLatestOrdersShipped] = useState([]);
  ...
  const fetchTeamMetrics = () => {
    ...
    const requestUrl = `${config.baseURL}/api-proxy/proxy?method=get&resource=partnermetrics`
    ...
    .then(response => {
      ...
      setLatestOrdersShipped(response.data.data.latest_orders_shipped);
    })
    .catch(error => {
  ...
  return (
    <Container maxWidth="xl">
      ...
      <Grid container spacing={3}>
        ...
        <Grid xs={12} md={4} lg={4}>
          <AppOrderTimeline
            title="Latest Shipped Orders"
            list={latestOrdersShipped.map((order, index) => ({
              id: faker.string.uuid(),
              title: order.name,
              type: `order${index + 1}`,
              ship_date: order.ship_date,
              ship_tracking_number: order.ship_tracking_number
            }))}
          />
        </Grid>
